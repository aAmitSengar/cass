...
require('react-json-pretty/dist/adventure_time');
...
class Result extends Component {
  ...
  handleRecordDelete = (indexToDelete, dataToDelete) => {
    let query = '';
    query =
      'delete from ' + this.props.keyspace + '.' + this.props.columnFamily;

    let conditions = '';
    this.props.columnDetails.map(obj => {
      if (
        (obj.kind === 'partition_key' || obj.kind === 'clustering') &&
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <Table id="table-to-xls">
          <TableHead>
            <TableRow style={{ height: '25px' }}>
              ...
              {Object.keys(this.props.resultDataReducer.result[0]).map(
                (value, index) => {
                  return (
                    <TableCell key={index}>
                      <b>{value}</b>
                  ...
                  <TableRow key={slNo} style={{ height: '20px' }}>
                    <TableCell>
                      ...
                      <Tooltip id="newConn" title=" Edit Record">
                        <EditIcon
                          onClick={event =>
                            this.props.flipEditDailog(data[currentIndex])
                          }
        ...
        >
          <DialogContent>
            <JSONPretty json={this.state.SelectedCellData} />
          </DialogContent>
        </Dialog>
      </div>
    );
