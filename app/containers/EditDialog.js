  ...
  handleOnClick = columnsAttributes => {
    ...
    Object.keys(data).map((field, index) => {
      if (
        ...
        condition +=
          this.getPairBasedOnType(
            columnsAttributes[field].type,
            field,
            data[field]
          ) + ' AND ';
      } else {
  ...
  updateRecord(queryStatement) {
    ...
    resp = CassandraAPICalls.fireUpdateQuery(queryStatement)
      ...
      .catch(error => {
        ...
        this.props.showNotification(
          true,
          'Update Failed ' + error.response.statusText,
          'error'
        );
      });
    console.log(resp);
  ...
  render() {
    ...
    return (
      <div>
        ...
        >
          <DialogTitle id="form-dialog-title" style={{ height: '14px' }}>
            Edit Record
          </DialogTitle>
          <DialogContent>
            <Grid container>
